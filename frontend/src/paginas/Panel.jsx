...
export default function Panel({ usuario }) {
  ...
  const cargarMisArticulos = () => {
    ...
    fetch(`http://localhost:8080/api/articulos/usuario/${usuarioId}`, {
      headers: { 'Authorization': `Bearer ${token}` }
        ...
        .then(res => {
          if (!res.ok) throw new Error('Error al cargar artículos del usuario');
          return res.json();
        })
        .then(data => setMisArticulos(Array.isArray(data) ? data : []))
  ...
  const handleEliminar = (id) => {
    ...
    fetch(`http://localhost:8080/api/articulos/${id}?idUsuario=${usuarioId}`, {
      method: 'DELETE',
      headers: { 'Authorization': `Bearer ${token}` }
    })
        .then(res => {
            ...
            <div className="form-group">
              ...
              <input
                  ...
                  onChange={async e => {
                    ...
                    try {
                      // Upload de imagen es público, no necesita token, pero no afecta si lo omites
