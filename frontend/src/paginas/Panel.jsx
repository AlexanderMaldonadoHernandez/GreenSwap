...
export default function Panel({ usuario }) {
  ...
  const handlePublicar = (e) => {
    ...
    if (!usuarioId) {
      alert('No se encontró el usuario. Cierra sesión e inicia sesión otra vez.');
      return;
    }
    ...
    fetch('http://localhost:8080/api/articulos', {
      ...
      .catch(error => {
        console.error(error);
        alert('Error: el artículo no se guardó. Revisa la consola o el backend.');
      });
  };
        ...
        <div className="card-grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))' }}>
          {misArticulos.map(art => (
            <div className="card" key={art.idArticulo}>
              ...
              <button className="btn btn-danger" onClick={() => handleEliminar(art.idArticulo)}>
                Eliminar
              </button>
            </div>
          ))}
